...
this.Searcher = (function() {
  ...
  function Searcher(recordType, batchSize, lowerBound, searchFilters, searchColumns) {
    ...
    this.SEARCH_COLUMN_SUMMARY_KEY = 'summary';    
    ...
    if(lowerBound)this.generateLowerBoundFilter();
    this.createSearchColumns();
    if(!this.sortSet && !this.summarySet)this.generateSortColumn();
    if(this.summarySet == true && this.summaryNotSet == true)throw 'All columns must have a summary parameter when one does.';
  }
  ...
  Searcher.prototype.generateSortColumn = function() {
    sortColumn = NetsuiteToolkit.searchColumn('internalid');
    sortColumn.setSort();
  ...
  Searcher.prototype.getSearchColumnObject = function(searchColumnData) {
    ...
    summary = searchColumnData[this.SEARCH_COLUMN_SUMMARY_KEY];
    column = NetsuiteToolkit.searchColumn(name, join, summary);
