this.Endpoint = (function () {
    ...
    var EXCEPTIONS = {
        ...
        PARAMETERS_MUST_BE_AN_ARRAY: 'The parameter \'params\' must be an array for this method'
    };
    ...
    Endpoint.prototype.handleRequest = function () {
        ...
        if (this._isFunction(this[method] || this.netsuiteApi[method])) {
            this.result = this[method] ? this._callMethod(method, params) : this._callNetsuiteApiMethod(method, params);
        } else {
    ...
    Endpoint.prototype._callMethod = function (method, params) {
        return this[method](params);
    };

    Endpoint.prototype._callNetsuiteApiMethod = function (method, params) {
        var result = null;

        if (params && !Array.isArray(params)) {
            this.addException(EXCEPTIONS.PARAMETERS_MUST_BE_AN_ARRAY);
        } else {
            result = this.netsuiteApi[method].apply(this.netsuiteApi, params || []);
        }

        return result;
    };

    Endpoint.prototype.getModuleFields = function (opts) {
        var self = this;

        if (opts.module) {
            var fields  = [];
            var records = NetsuiteToolkit.searchRecord(opts.module);

            if (records.length) {
                var record     = NetsuiteToolkit.loadRecord(opts.module, records[0].id);
